// ══════════════════════════════════════════════════════════════════════════
// Familia Burgada — Script v5
// ══════════════════════════════════════════════════════════════════════════

// ⚠️ CAMBIAR ESTE EMAIL por el real del administrador
const ADMIN_EMAIL = 'albertoburgada@gmail.com';
...
let activeLineagePersonIds = null, activeLineageEdges = null;
...
function formatDates(p) {
  const b = p.birthDate || '', d = p.deathDate || '';
  if (!b && !d) return '';
  return d ? `${b} – ${d}` : `${b} –`;
}
...
function avatarHTML(person, size) {
  ...
  if (person.photoUrl) {
    return `<img class="ft-avatar-img" style="width:${s}px;height:${s}px" src="${person.photoUrl}" alt="${person.name}" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'" />
            <div class="ft-avatar-initials" style="width:${s}px;height:${s}px;display:none;background:${g.bg};color:${g.color}">${getInitials(person.name)}</div>`;
  }
...
function buildFamilyTree() {
  ...
  if (roots.length === 1) tree = buildSubTree(roots[0], null);
    ...
    tree = { id: -1, virtual: true, person: null, partner: null, isMarriage: false, children: roots.map(r => buildSubTree(r, null)), collapsed: false, generation: 0, parentTreeNode: null };
    tree.children.forEach(c => c.parentTreeNode = tree);
  }
  treeNodeByPersonId = new Map();
  indexTreeNodes(tree);
  return tree;
}
...
function getLineageNames(personId) {
  ...
  if (tn) {
    let w = tn.parentTreeNode;
    while (w) { if (!w.virtual && w.person) names.push(w.person.name); w = w.parentTreeNode; }
  }
  return names;
}

function activateLineage(personId) {
  const { personIds, edges } = computeLineagePath(personId);
  activeLineagePersonIds = personIds;
  activeLineageEdges = edges;
  const names = getLineageNames(personId);
  document.getElementById('ft-lineage-text').textContent = '🌳 ' + names.join('  →  ');
  document.getElementById('ft-lineage-bar').classList.remove('hidden');
  applyVisualState();
}

function clearLineage() {
  activeLineagePersonIds = null; activeLineageEdges = null;
  document.getElementById('ft-lineage-bar').classList.add('hidden');
  applyVisualState();
}
...
function makeCollapseBtn(node) {
  ...
  btn.className = 'ft-collapse-btn'; btn.title = node.collapsed ? 'Expandir' : 'Colapsar'; btn.textContent = node.collapsed ? '+' : '−';
...
function renderNode(node, canvas, order) {
  ...
  if (node.isMarriage) {
    ...
    c2.style.borderTopColor = '#b8ad98'; c2.innerHTML = createPersonCard(node.partner);
    c2.addEventListener('click', e => { if (!e.target.closest('.ft-collapse-btn')) openModal(node.partner); });
    registerEl(node.partner.id, c2);
...
function renderLevelLabels(depthCount) {
  ...
  if (!colWrap?.classList.contains('ft-col-wrap')) {
    const wrap = document.createElement('div'); wrap.className = 'ft-col-wrap';
  ...
  if (!bar) {
    bar = document.createElement('div'); bar.className = 'ft-level-bar'; bar.innerHTML = '<div class="ft-level-inner"></div>';
    colWrap.insertBefore(bar, viewport);
    viewport.addEventListener('scroll', () => { const inn = bar.querySelector('.ft-level-inner'); if (inn) inn.style.transform = `translateX(-${viewport.scrollLeft}px)`; });
  }
  bar.style.display = '';
  const inner = bar.querySelector('.ft-level-inner'); inner.innerHTML = '';
  for (let d = 0; d <= depthCount; d++) {
    const lbl = document.createElement('div'); lbl.className = 'ft-level-label';
    lbl.style.width = `${CARD_W + COL_GAP}px`; lbl.style.flexShrink = '0';
    lbl.textContent = `Generación ${d}`; lbl.style.color = genStyle(d).color; inner.appendChild(lbl);
  }
}
...
function applyVisualState() {
  ...
  const hasSearch = term !== '' || gf !== 'all', hasLineage = activeLineagePersonIds != null;

  for (const [pid, elements] of personElements) {
    const person = peopleById.get(pid); if (!person) continue;
    let dim = false;
    if (hasLineage && !activeLineagePersonIds.has(pid)) dim = true;
    if (hasSearch) {
      const okT = !term || (person.name || '').toLowerCase().includes(term);
      const okG = gf === 'all' || String(person.generation ?? '') === gf;
      if (!okT || !okG) dim = true;
    }
    for (const el of elements) {
      el.classList.toggle('ft-dimmed', dim);
      el.classList.toggle('ft-highlight', hasSearch && !dim);
      el.classList.toggle('ft-lineage-active', hasLineage && !dim);
    }
  }

  for (const [key, pathEl] of edgeElements) {
    if (hasLineage) {
      const inL = activeLineageEdges?.has(key);
      pathEl.setAttribute('stroke', inL ? '#6f5b3e' : '#e8e3da');
      pathEl.setAttribute('stroke-width', inL ? '3' : '1.5');
      pathEl.style.opacity = inL ? '1' : '0.3';
    } else {
      pathEl.setAttribute('stroke', '#b8ad98'); pathEl.setAttribute('stroke-width', '2'); pathEl.style.opacity = '1';
    }
  }
}
...
function openModal(person) {
  ...
  if (person.photoUrl) {
    ac.innerHTML = `<img class="ft-modal-photo" src="${person.photoUrl}" alt="${person.name}" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'" /><div class="ft-avatar-initials ft-avatar-initials-lg" style="display:none;background:${gen.bg};color:${gen.color}">${getInitials(person.name)}</div>`;
  } else {
    ac.innerHTML = `<div class="ft-avatar-initials ft-avatar-initials-lg" style="background:${gen.bg};color:${gen.color}">${getInitials(person.name)}</div>`;
  }
  ...
  badge.style.background = isFam ? gen.bg : '#f0ece4'; badge.style.color = isFam ? gen.color : '#8a7e6e';
...
(async function init() {
  ...
  document.getElementById('modal-trace-lineage').addEventListener('click', () => { if (_modalPerson) { bd.classList.remove('open'); activateLineage(_modalPerson.id); } });
  document.getElementById('ft-lineage-clear').addEventListener('click', clearLineage);
